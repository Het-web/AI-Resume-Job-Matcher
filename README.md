...
This project uses **Supabase Storage** to securely store uploaded resume PDFs.
...
4. Add Storage Row Level Security (RLS) policies to allow uploads and signed URL generation for your application.
5. Add a bucket policy allowing INSERT and SELECT to ANON role.

> **Note:** The `resumes` bucket should remain **private**. The frontend uploads the PDF using the Supabase Publishable Key, and n8n accesses the file through a temporary signed URL generated by the frontend. No public access to uploaded resumes is required.
